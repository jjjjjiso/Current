...
namespace WaterBlast.Game.Common
{
    ...
    public partial class Stage : MonoBehaviour
    {
        ...
        private Coroutine co_sticky;
        private Coroutine co_radiation;

        bool isEndCheck;

        LevelBlockType lbtSticky = new LevelBlockType() { type = BlockType.sticky };
        LevelBlock lbtRadiation = new LevelBlock() { blockerType = BlockerType.radiation };
        ...
        public void NormMatch(BlockEntity pickBlock)
        {
            ...
            if (matchedBlocks.Count >= 2) //2개 이상 터트리기.
            {
                ...
                {
                    ReturnBlocker(matchedBlockers[i]);
                }
                ...
                if (matchedBlocks.Count < 5)
                {
                    ...
                    if (isEndCheck) GameMgr.G.GameEnd();
                }
                else
                {
        ...
        public void BoosterMatches(BlockEntity pickBlock, bool isComboCheck = true)
        {
            ...
            matchedBlocks = pickBooster.Match(pickBooster._X, pickBooster._Y);
            pickBooster._IsSelect = true;
        ...
        private void UpdateCurStickyBlockCount(Block block)
        {
            if (block == null) return;
            if (block._BlockType == BlockType.sticky)
            {
                --currStickyBlockCount;
                if (currStickyBlockCount <= 0) currStickyBlockCount = 0;
            }
        }

        private void StartCheckSticky()
        {
            if (co_sticky != null)
            {
                StopCoroutine(co_sticky);
                co_sticky = null;
            }
            co_sticky = StartCoroutine(Co_CheckStickyBlock());
        }
                ...
                foreach (BlockEntity entity in blockEntities)
                {
                    ...
                    foreach (var temp in surroundingBlocks)
                    {
                        if (IsValidBlockEntity(temp))
                        {
                            ...
                            RemoveCollectedBlock(lbtSticky);
                            yield break;
                        }
                    }
                }
        ...
        private void StartCheckRadiation()
        {
            if (co_radiation != null)
            {
                StopCoroutine(co_radiation);
                co_radiation = null;
            }
            co_radiation = StartCoroutine(Co_CheckRadiationBlocker());
        }
                ...
                foreach (BlockEntity entity in blockEntities)
                {
                    ...
                    foreach (var temp in surroundingBlocks)
                    {
                        if (IsValidBlockEntity(temp))
                        {
                            ...
                            RemoveCollectedBlock(lbtRadiation);
                            yield break;
                        }
                    }
                }
        ...
        IEnumerator Co_BoosterMatch(List<BlockEntity> boosters, BoosterType type, bool isComboCheck = true)
        {
            ...
            if (defCount != 0)
            {
                while (count < defCount)
                {
                    ...
                    if(!hitBlock.gameObject.activeSelf || IsCheckBlock(hitBlock, BlockType.empty))
                    {
                        ++count;
                        continue;
                    }
                    ...
                    {
                        // 블록커 제거
                        ReturnBlocker(blockerEntites[hitBlock._X, hitBlock._Y]);
                        ++count;
                        continue;
                    }

                    UpdateCurStickyBlockCount(hitBlock as Block);

                    hitBlock.Hide();
                    Booster booster = hitBlock as Booster;
                    if (booster != null)
                    {
                        if (!booster._IsSelect)
                        {
                            if (!booster._IsCombo)
                            {
        ...
        IEnumerator Co_UseHammerItem(BlockEntity block)
        {
            ...
            {
                UpdateCurStickyBlockCount(block as Block);
                block.Hide();
        ...
        IEnumerator Co_UseMittHorizonItem(int y)
        {
            ...
            while (x < width)
            {
                ...
                {
                    ReturnBlocker(blockerEntites[blockEntity._X, blockEntity._Y]);
                }
                else
                {
                    ...
                    if (block != null)
                    {
                        if (block._BlockType != BlockType.can && block._BlockType != BlockType.paper)
                        {
                            //미션 체크.
                            AddCollectedBlock(blockEntity);
                            UpdateCurStickyBlockCount(block);
                            block.Hide();
            ...
            foreach (var surroundingBlock in surroundingBlocks)
            {
                if (IsValidBlockEntity(surroundingBlock))
                {
                    ...
                    if (tempBlockEntity != null && tempBlockEntity.gameObject.activeSelf)
                    {
                        if (isColorMatch)
                        {
                            Block block = tempBlockEntity as Block;
                            Blocker blocker = blockerEntites[tempBlockEntity._X, tempBlockEntity._Y] as Blocker;
                            if (block != null)
                            {
                        ...
                        {
                            Booster booster = tempBlockEntity as Booster;
        ...
        List<BlockEntity> stack;
        List<int> emptyIdxs;
        Vector2 vEndPos;
        Vector2 vTemp;
        private bool MatchDown()
        {
            bool isEndCheck = true;
            if (stack == null) stack = new List<BlockEntity>();
            if (emptyIdxs == null) emptyIdxs = new List<int>();
            vEndPos = Vector2.zero;
            vTemp = Vector2.zero;
            for (int x = 0; x < width; ++x)
            {
                stack.Clear();
                ...
                for (int y = 0; y < height; ++y)
                {
                    ...
                    if (blockEntity._State == State.wait)
                    {
                        if (!stack.Contains(blockEntity))
                        {
                            ...
                            vTemp.x = Point(x, width, wSize);
                            vTemp.y = (15 * hSize);
                            blockEntity._LocalPosition = vTemp;
                        }
                    }
                    else if (stack.Count != 0)
                    {
                        ...
                        int yy = (y - stack.Count) - emptyIdxs.Count;
                        yy = GetEmptyMoveY(yy);

                        Swap(x, y, x, yy);
                        vEndPos.x = Point(x, width, wSize);
                        vEndPos.y = Point(yy, height, hSize);
                ...
                {
                    int y = ((height - 1) - i) - emptyIdxs.Count;
                    y = GetEmptyMoveY(y);
                    var blockEntity = blockEntities[x, y];
                    blockEntity.Show();
                    vEndPos.x = Point(x, width, wSize);
                    vEndPos.y = Point(y, height, hSize);
        ...
        int GetEmptyMoveY(int y)
        {
            if (y < 0) y = 0;
            for (int e = 0; e < emptyIdxs.Count; ++e)
            {
                if (emptyIdxs[e] <= y) ++y;
            }
            return y;
        ...
        public void BlockIconSetting(bool init = false)
        {
        ...
        IEnumerator Co_BlockerSetting()
        {
            ...
            for (int x = 0; x < width; ++x)
            {
                for (int y = 0; y < height; ++y)
                {
                    ...
                    {
                        Booster booster = blockEntities[topBlock.x, topBlock.y] as Booster;
        ...
        private void CreateBooster(GameObject booster, int x, int y)
        {
            ...
            Booster tmpBooster = entity as Booster;
            tmpBooster._IsCombo = false;
            tmpBooster._IsSelect = false;
        }
        ...
        private void ReturnBlocker(BlockEntity blockEntity, bool isCheck = true)
        {
            ...
            else if (blocker._BlockerType == BlockerType.radiation)
            {
                ...
                --currRadiationCount;
                if (currRadiationCount <= 0) currRadiationCount = 0;
        ...
        private void ReturnBlockerObject(Blocker blocker)
        {
            ...
            GameObject particles = GameMgr.G.gamePools.GetBlockerParticles(blocker);
        ...
        private bool IsCheckBlocker(int x, int y, BlockerType type)
        {
            if (blockerEntites[x, y] != null && (blockerEntites[x, y] as Blocker)._BlockerType == type) return true;
            return false;
        }
        ...
        public void RemoveCollectedBlock(LevelBlock type)
        {
            GameMgr mgr = GameMgr.G;
            int count = 0;
            if (type is LevelBlockType)
            {   //block
                BlockType blockType = (type as LevelBlockType).type;
                if (GameMgr.G._GameState.collectedBlocks.ContainsKey(blockType))
                {
                    count = mgr._GameState.collectedBlocks[blockType] - 1;
                    if (count < 0)
                    {
                        GameMgr.G.UpdateGoalUI(type);
                        foreach (Goal goal in mgr.level.goals)
                        {
                            if (goal is CollectBlockGoal)
                            {
                                CollectBlockGoal blockGoal = goal as CollectBlockGoal;
                                blockGoal.amount_plus += 1;
                            }
                        }
                    }
                    else GameMgr.G._GameState.collectedBlocks[blockType] -= 1;
                }
            }
            else if (type is LevelBlock)
            {   //blocker
                BlockerType blockerType = (type as LevelBlock).blockerType;
                if (mgr._GameState.collectedBlockers.ContainsKey(blockerType))
                {
                    count = mgr._GameState.collectedBlockers[blockerType] - 1;
                    if (count < 0)
                    {
                        GameMgr.G.UpdateGoalUI(type);
                        foreach (Goal goal in mgr.level.goals)
                        {
                            if (goal is CollectBlockerGoal)
                            {
                                CollectBlockerGoal blockerGoal = goal as CollectBlockerGoal;
                                blockerGoal.amount_plus += 1;
                            }
                        }
                    }
                    else GameMgr.G._GameState.collectedBlockers[blockerType] -= 1;
                }
            }
